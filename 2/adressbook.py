...
import pickle
import re
from src.seed import create_names_contacts, create_emails_contacts, create_phones_contacts
...
class Email(Field):
    ...
    def value(self, value: str):
        result = None
        pattern = r'\b[a-zA-Z][\w\.]+@[a-zA-Z]+\.[a-zA-Z]{2,}'
        get_email = re.findall(pattern, value)
        for i in get_email:
            result = i
        if result is None:
...
def add_email(contacts, *args):
    ...
    writing_db(contacts)
    create_emails_contacts(n_email=email)
...
def add_address(contacts, *args):
    ...
    writing_db(contacts)
    create_names_contacts(n_name=name, n_address=address)
...
def add_birthday(contacts, *args):
    ...
    writing_db(contacts)
    create_names_contacts(n_name=name, n_birthday=birthday)
